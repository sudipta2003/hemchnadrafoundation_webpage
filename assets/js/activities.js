...
function renderPhotosCarousel(shouldAnimate = false) {
	...
	function renderNewPhotos() {
		...
		for (let i = 0; i < PHOTOS_PER_PAGE; i++) {
			...
			articlesHTML += `
				<article class="box excerpt activity-card-responsive ${animationClass}" style="display: flex; flex-direction: row; align-items: center; gap: 20px; padding: 20px; border: 3px solid ${cardBorderColor}; border-radius: 8px; background-color: ${cardColor}; box-shadow: 0 2px 8px rgba(0,0,0,0.1); margin-bottom: 20px; transition: box-shadow 0.3s ease, transform 0.3s ease; ${animationStyle}" onmouseover="this.style.boxShadow='0 8px 20px rgba(0,0,0,0.2)'; this.style.transform='translateY(-5px)';" onmouseout="this.style.boxShadow='0 2px 8px rgba(0,0,0,0.1)'; this.style.transform='translateY(0)';">
